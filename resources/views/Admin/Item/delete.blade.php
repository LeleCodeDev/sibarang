<dialog id="delete_modal_{{ $item->id }}" class="modal">
    <div class="modal-box">
        <form method="POST" action="{{ route('category.destroy', $item->id) }}">
            ...
            <h3 class="font-bold text-xl mb-4 text-center">Are you sure to delete this item ?</h3>

            <div class="w-full flex justify-center gap-2">
                <button type="submit" class="btn btn-error ">Delete</button>
                <button type="button" class="btn" onclick="delete_modal_{{ $item->id }}.close()">Cancel</button>
            </div>
        </form>
    </div>
</dialog>
