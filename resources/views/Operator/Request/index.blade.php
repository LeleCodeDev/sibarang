            ...
            <tbody>
              @forelse ($borrowRequests as $request)
                <tr class="hover">
                  ...
                  <td class="align-top">
                    <div class="flex items-center gap-4 p-2   rounded-lg  hover:shadow-md transition-shadow">
                      <div class="avatar">
                      ...
                      <div class="flex flex-col justify-center gap-2">
                        <p class="font-semibold text-sm">
                          {{ $request->item->name ?? 'Unknown Item' }}</p>
                        <span class="badge badge-primary badge-sm w-fit">{{ $request->quantity }}x</span>
                      </div>
                    </div>
                  </td>
                        ...
                        @if ($request->status == 'processed')
                          <li>
                            <form action="{{ route('borrow-request.approve', $request->id) }}" method="POST">
                              @csrf
                              @method('PUT')
                              <button type="submit" class="flex items-center gap-2 text-success w-full">
                        ...
                        @if ($request->status == 'approved')
                          <li>
                            <form action="{{ route('borrow-request.return', $request->id) }}" method="POST">
                              @csrf
                              @method('PUT')
                              <button type="submit" class="flex items-center gap-2 text-info w-full">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                  stroke-linecap="round" stroke-linejoin="round">
